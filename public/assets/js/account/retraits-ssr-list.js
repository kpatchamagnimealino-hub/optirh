...
$(document).ready(function() {
    ...
    function loadData(initial_date, finale_date) {
        ...
        if (isDataTableInitialized('#retraits_table')) {
            $('#retraits_table').DataTable().destroy();
        }

        // Initialisation de DataTable
        var table = new DataTable('#retraits_table', {
            responsive: true,
            serverSide: true,
            paging: true,
            processing: true,
            ...
            sort: true,
            ajax: {
                url: "/accounts/retraits/ssr-list",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': TOKEN
                },
                data: {
                    "retrait_from_date": initial_date,
                    "retrait_to_date": finale_date,
    ...
    $('form#retrait_date_filter').on('submit', function(e) {
        e.preventDefault();
        var from_date = $('#retrait_from_date').val();
        var to_date = $('#retrait_to_date').val();
        loadData(from_date, to_date);
    });

    $('#retrait_from_date').on('change', function() {
        $('#retrait_to_date').removeAttr("readonly");
        $('#retrait_to_date').attr("min", $(this).val());
        $('#retrait_to_date').val($(this).val());
    });
});

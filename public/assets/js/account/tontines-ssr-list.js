...
$(document).ready(function() {
    ...
    function loadData(initial_date, finale_date) {
        ...
        if (isDataTableInitialized('#tontines_table')) {
            $('#tontines_table').DataTable().destroy();
        }

        // Initialisation de DataTable
        var table = new DataTable('#tontines_table', {
            responsive: true,
            serverSide: true,
            paging: true,
            processing: true,
            ...
            sort: true,
            ajax: {
                url: "/accounts/tontines/ssr-list",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': TOKEN
                },
                data: {
                    "tontine_from_date": initial_date,
                    "tontine_to_date": finale_date,
    ...
    $('form#tontine_date_filter').on('submit', function(e) {
        e.preventDefault();
        var from_date = $('#tontine_from_date').val();
        var to_date = $('#tontine_to_date').val();
        loadData(from_date, to_date);
    });

    $('#tontine_from_date').on('change', function() {
        $('#tontine_to_date').removeAttr("readonly");
        $('#tontine_to_date').attr("min", $(this).val());
        $('#tontine_to_date').val($(this).val());
    });
});

...
$(document).ready(function() {
    ...
    function loadData(initial_date, finale_date) {
        ...
        if (isDataTableInitialized('#restitutions_table')) {
            $('#restitutions_table').DataTable().destroy();
        }

        // Initialisation de DataTable
        var table = new DataTable('#restitutions_table', {
            responsive: true,
            serverSide: true,
            paging: true,
            processing: true,
            ...
            sort: true,
            ajax: {
                url: "/accounts/restitutions/ssr-list",
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': TOKEN
                },
                data: {
                    "restitution_from_date": initial_date,
                    "restitution_to_date": finale_date,
    ...
    $('form#restitution_date_filter').on('submit', function(e) {
        e.preventDefault();
        var from_date = $('#restitution_from_date').val();
        var to_date = $('#restitution_to_date').val();
        loadData(from_date, to_date);
    });

    $('#restitution_from_date').on('change', function() {
        $('#restitution_to_date').removeAttr("readonly");
        $('#restitution_to_date').attr("min", $(this).val());
        $('#restitution_to_date').val($(this).val());
    });
});
